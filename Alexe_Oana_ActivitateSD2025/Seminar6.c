...
void adaugaMasinaInLista(Nod* cap, Masina masinaNoua) //Nod ** cap e pointerul care retine adresa pointer-ului in care e adresa primului nod
{
	if (cap)
	{
...
int main() {
	...
	Masina m = getMasinaDupaCheie(ht, "Ionel");
	printf("Cautam masina lui Ionel: \n");
	if (m.id != -1)
	{
		afisareMasina(m);
		free(m.model);
