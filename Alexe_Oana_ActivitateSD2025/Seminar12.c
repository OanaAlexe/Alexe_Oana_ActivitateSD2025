...
//sau va creati un alt fisier cu alte 

//arbore este un graf conex si aciclic
...
NodListaSecundara* inserareaListaSecundara(NodListaSecundara* cap, NodListaPrincipala* info) {
...
void inserareMuchie(NodListaPrincipala* listaPrincipala, int idStart, int idStop) {
	...
	if (nodStart && nodStop) {
		nodStart->vecini = inserareaListaSecundara(nodStart->vecini, nodStop);
		nodStop->vecini = inserareaListaSecundara(nodStop->vecini, nodStart);
	}
}
...
NodListaPrincipala* citireNoduriMasiniDinFisier(const char* numeFisier) {
	...
	while (!feof(file)) {
		inserareaListaPrincipala(listaPrincipala, citireMasinaDinFisier(file));
	}
	fclose(file);
	return listaPrincipala;
}
...
int main() {
	...
	//lucrare din heap, arbori, stive, cozi, grafuri
	return 0;
}